...
int useMeanTherm = 0;
int useMeanFlat = 0;
int useUnscaledThermal = 0; // Turn only ONLY for an RBI Flush mitigation. Normally, we want to scale.
...
void readCorrectionCfg(int trace, char *cfgFile)
{
    static CfgEntry ccfg[] =
    {
		...
		{"UNSCALEDTHERMAL", CFG_INT, &useUnscaledThermal}
    };
 ...
 * fill fn with full name and return 0, else fill errmsg and return -1.
 */
int findMapFN (caldir, fn, errmsg)char caldir[];
char fn[];
char errmsg[];
{
...
nc_applyThermal(int n, float *acc, double dur, char thermfn[], char msg[])
{
    ...
    if (n > w*h)
    {
     ...
     // STO: July 8, 2011 -- Allow option to NOT scale thermal (RBI Flush mitigation)
    if(useUnscaledThermal) {
        tdur = dur;
    } else {
