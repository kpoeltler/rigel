...
/*
 * STO: March 2011 - Removing this in favor of FITS standard use of DATE-OBS field
...
// RETURN NON-ZERO IF WE WANT SEPARATE DATE-OBS/TIME-OBS FIELDS OR ELSE A SINGLE ISO8601 DATE-OBS STANDARD.
int useTimeObsField()
{
    return 0;
}
...
/*
 * Replacement for timeStampFITS that uses DATE-OBS in according to FITS standard.
 * fip->dur is expected to have been set prior to calling this.
 */
void timeStampExposureStartFITS(FImage* fip)
{
    struct tm *tmp;
    struct timeval tv;
    char buf[100];
    ...
    tv = computeTimeStampNow((double) fip->dur/1000.0);
    tmp = gmtime (&tv.tv_sec);

    day = tmp->tm_mday +
          (tmp->tm_hour + (tmp->tm_min + (tmp->tm_sec + tv.tv_usec/1000000.0)/60.0)/60.0)/24.0;
    cal_mjd (tmp->tm_mon+1, day, tmp->tm_year+1900, &mjd);
    setRealFITS (fip, "JD", mjd+MJD0, 16, "Julian Date, start of exposure");

    getISO8601DateObsValue(tv, buf, sizeof(buf)/sizeof(buf[0]));

    // If we are set up to use TIME-OBS and DATE-OBS, separate these values
    if (useTimeObsField())
    {
        buf[10] = 0;
    }
    setStringFITS(fip, "DATE-OBS", buf, "UTC, start of exposure");
    if (useTimeObsField())
    {
        setStringFITS(fip, "TIME-OBS", buf+11, "UTC, start of exposure");
    }
}

/* STO, March 2011
 * Compute the date and time of the current exposure AT THE BEGINNING OF THE EXPOSURE
 * given a call to this immediately at the end of the exposure (per normal Talon call flow).
 */
struct timeval
computeTimeStampNow(double exposureSeconds)
{
    struct timeval tv;

    gettimeofday(&tv, NULL);

    double rusec = (double) tv.tv_sec + (double) tv.tv_usec / 1000000.0;
    rusec -= exposureSeconds;
    long lsecs = floor(rusec);
    long usecs = (rusec - (double) lsecs) * 1000000.0;
    tv.tv_sec = (time_t) lsecs;
...
/* STO, March 2011
 * Format the given timeval into ISO8601 format, per FITS DATE-OBS standard.
 */
void
getISO8601DateObsValue(struct timeval tv, char* outString, int outStringLen)
{
    ...
    sprintf (buf, "%4d-%02d-%02dT%02d:%02d:%02d.%03d", 1900+tmp->tm_year, tmp->tm_mon+1,
             tmp->tm_mday ,tmp->tm_hour,tmp->tm_min,tmp->tm_sec, (int) (tv.tv_usec/1000));

    if (!outString || outStringLen < 1) return;
    strncpy(outString, buf, outStringLen);
}
