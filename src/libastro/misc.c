...
/*
 * STO, March 2011
 * Compute a spherical angular distance in radians,
 * ax,ay and bx, by represent "ra" and "dec" or "lat", "long" coordinates, expressed
 * in terms of radians.
 *
 * d = arccos(sin(ax)*sin(bx)+cos(ax)*cos(bx)*cos(by-ay))
 *
 */
double
angularDistance(double ax, double ay, double bx, double by)
{
    double x = sin(ax)*sin(bx)+cos(ax)*cos(bx)*cos(by-ay);
    return acos(x);
}

/* STO March, 2011
 * Compute the angular distance from the given alt, az to the risen moon
 * Value is -1 if the moon is below the horizon.
 * All values in radians
 */
double moonAngle(Now* np, double alt, double az)
{
    Obj moonobj;

    // calc moon phase and position
    (void) memset ((void *)&moonobj, 0, sizeof(moonobj));
    moonobj.o_type = PLANET;
    moonobj.pl.pl_code = MOON;
    (void) obj_cir (np, &moonobj);
    if (moonobj.s_alt < 0) return -1.0; // if moon is below horizon

    return angularDistance(az, alt, moonobj.any.co_az, moonobj.any.co_alt);
}

/* STO March, 2011
 * Return the phase of the moon (percentage)
 */
double moonPhase(Now* np)
{
    Obj moonobj;
