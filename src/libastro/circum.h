...
extern int db_crack_line_name P_((char s[], Obj *op, char whynot[], int nameSubfield));
...
extern double angularDistance(double ax, double ay, double bx, double by);
extern double moonAngle(Now* np, double alt, double az);
extern double moonPhase(Now* np);
