...
void get_nth_subfield(int n, char* s, int delim, char* field, int maxChars);
...
char whynot[];
{
    return db_crack_line_name (s, op, whynot, 0);
}
int
db_crack_line_name (s, op, whynot, nameSubfield)
char s[];
Obj *op;
char whynot[];
int nameSubfield;
{
    ...
    // Get the correct subfield of the name entry
    get_nth_subfield(nameSubfield, copy, SUBFLD, flds[0], strlen(flds[0]));
...
/* Get the nth subfield from the given string and fill into the given field buffer */
void get_nth_subfield(int n, char* s, int delim, char* field, int maxChars)
{
    int f=0;
    while (f < n)
    {
        while (*s && *s != FLDSEP && *s != delim) s++;
        f++;
        if (*s == delim) s++;
    }
    f = 0;
    while (*s && *s <= 32) s++;
    while (f < maxChars && *s && *s != FLDSEP && *s != delim)
    {
        field[f++] = *s++;
    }
    field[f] = 0;
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: dbfmt.c,v $ $Date: 2001/04/19 21:12:13 $ $Revision: 1.1.1.1 $ $Name:  $"};
