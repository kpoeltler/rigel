...
int initCCD (char *path, char* idkey, int auxcam, char *msg);
 ...
 * out automatically.
 *
 * Latest change (1/2010): IDKEY can now be passed, allowing a selection
 * from multiple installed cameras if needed.
 * This may be necessary, for example, when using an autoguider.
 *
 ...
 */
int setPathCCD (char *path, char* idkey, int auxcam, char *msg)
{
    ...
    if (first_time)
    {
        saved_retval = initCCD(path, idkey, auxcam, saved_msg);
        first_time = 0;
    }
 ...
 * This version sets the global "callbacks" instance,
 * per the "main" camera.
 * See initCCDInstance() for version that inits an independent,
 * preallocated CCDCallbacks structure.
 */
int
initCCD (char *path, char* idkey, int auxcam, char *msg)
{
    if (callbacks)
    {
        free(callbacks);
    ...
    callbacks = (CCDCallbacks *)malloc(sizeof(CCDCallbacks));

    int rt = initCCDInstance(path, auxcam, msg, callbacks, idkey);
...
/* This should be called for any auxillary camera initialization.
 * (such as in the case of an autoguider camera)
 * Preallocate and maintain the CCDCallbacks structure as the
 * auxillary camera instance reference, and call the
 * functions from these pointers directly. Do not use the
 * CCDCamera API for the aux camera, as it refers directly
 * to the main camera only.
 * The idkey parameter is used to select a specific camera by matching
 * a term in its ID string.  The key is matched on a substring detection.
 * It is ignored if null or empty.
 */
int
initCCDInstance(char* path, int auxcam, char* msg, CCDCallbacks* cbInstance, char* idKey)
{
    ...
    CCDCallbacks cb;
    int detected = 0;
    int show = idKey ? !strcmp(idKey,"@SHOW@") : 0;
...
//      printf("Trying driver %d of %d...\n", i+1, num_drivers);
        ...
        {
            memcpy(cbInstance, &cb, sizeof(cb));
            if (0 == cbInstance->getIDCCD(fixpath, msg)) // use fixpath buffer, since we have it handy
            {
                if (show) // If caller specified "@SHOW@", print ids found to stdout
                {
                    printf("%s\n", fixpath);
                }
                detected++;
                if (idKey == NULL || idKey[0] == 0 || strstr(fixpath, idKey))
                {
//                  printf("Camera matches request. Selecting.\n");
                    return 0;
                }
            }
            else
            {
                printf("error on camera id\n");
            }
        }
    }

    if (!detected)
        strcpy(msg, "No cameras detected");
    else
        sprintf(msg,"%d camera%s detected, none selected", detected, detected == 1 ? "" : "s");
