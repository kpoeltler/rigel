...
aux_findCCD (char *path, char *errmsg)
{
    ...
    n = 0;
    sprintf (argv[n] = buf[n], "%s", basenm(aux_cmd));
    n++;
    sprintf (argv[n] = buf[n], "-f");
    n++;
    argv[n] = NULL;
...
aux_setExpCCD (CCDExpoParams *expP, char *errmsg)
{
    ...
    n = 0;
    sprintf (argv[n] = bufs[n], "%s", basenm(aux_cmd));
    n++;
    sprintf (argv[n] = bufs[n], "-g");
    n++;
...
aux_startExpCCD (char *errmsg)
{
    ...
    if (TIME_SYNC_DELAY)
    {
        sprintf (argv[n] = bufs[n], "%ld",time(NULL)+TIME_SYNC_DELAY);
        n++;
    }
    argv[n] = NULL;
...
aux_setTempCCD (CCDTempInfo *tp, char *errmsg)
{
    ...
    sprintf (argv[n] = buf[n], "%s", basenm(aux_cmd));
    n++;
    sprintf (argv[n] = buf[n], "-t");
    n++;
    // STO 10-22-02: Support "OFF" as well
    if (tp->s == CCDTS_OFF)
    {
        sprintf(argv[n] = buf[n],"%s","OFF");
        n++;
    }
    else
    {
        sprintf (argv[n] = buf[n], "%d", tp->t);
        n++;
    }
    argv[n] = NULL;
...
aux_setShutterNow (int open, char *errmsg)
{
    ...
    sprintf (argv[n] = buf[n], "%s", basenm(aux_cmd));
    n++;
    sprintf (argv[n] = buf[n], "-s");
    n++;
    sprintf (argv[n] = buf[n], "%d", open);
    n++;
    argv[n] = NULL;
...
aux_getIDCCD (char buf[], char *errmsg)
{
    ...
    n = 0;
    sprintf (argv[n] = cbuf[n], "%s", basenm(aux_cmd));
    n++;
    sprintf (argv[n] = cbuf[n], "-i");
    n++;
    argv[n] = NULL;
