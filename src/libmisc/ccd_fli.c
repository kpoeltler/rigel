...
//  fliframe_t frametype;
    ...
    if (FLISetFrameType (fli_dev, FLI_FRAME_TYPE_DARK) != 0)
    {
        sprintf (errmsg, "Can not set FLI frame type to FLI_FRAME_TYPE_DARK");
        return (-1);
    }
    ...
    {
        sprintf(errmsg, "Can not set FLI exposure time to %d for bias drain", FLI_MIN_EXPOSURE_TIME);
        return (-1);
    }

    printf("RBI draining with %d bias frames\n",cnt);
