...
// Initialization function to set up the CCDCallbacks structure.  Used
// both by setPathCCD / initCCD() for the main camera as well as any
// specific initialization required for auxillary cameras.
extern int initCCDInstance(char* path, int auxcam, char* msg, CCDCallbacks* cbInstance, char* idKey);
...
extern int setPathCCD (char *path, char* idkey, int auxcam, char *errmsg);
