...
static void matchByDist (double sx[], double sy[], int ns,
                         double gr[], double gd[], double gx[], double gy[], int ng,
                         double x2as, double y2as, double pMATCHDIST, int nenough, int npmax,
                         int mats[], int matg[], int *np);
static void matchByPos (double sx[], double sy[], int ns,
                        double gr[], double gd[], double gx[], double gy[], int ng,
                        double xsc, double ysc, double pMATCHDIST, int npmax,
                        int mats[], int matg[], int *np);
...
findRegistrationD (fip, ra0, dec0, rot0, psx0, psy0, sx, sy, ns, gr, gd, ng,
                   nparam, pMINPAIR, pTRYSTARS, pMAXROT, pMATCHDIST, rhp, rp)
...
int pMINPAIR;      /* min pairs for WCS fit */
int pTRYSTARS;     /* try fit if find this no. of pairs, don't look for more */
double pMAXROT;    /* max rotation in degrees, for WCS */
double pMATCHDIST; /* limit (arcsec) within which distances are considered
                     ...
                     OUT: actual mean residual (pixels) */
{
    ...
    if (nparam == 5)
        matchByDist (sx, sy, ns, gr, gd, gx, gy, ng,
                     raddeg(t_sx)*3600, raddeg(t_sy)*3600, pMATCHDIST,
                     pTRYSTARS, npmax,  mats, matg, &npair);
    else
...
out:
     ...
     * and scaling
     */
    if (ok && resid_max < *rp  && acos(cos(t_th)) < degrad(pMAXROT)
...

    {
        ...
        a[1] = ct*t_sx;
        a[2] = -st*t_sy;
        a[3] = -ct*t_sx*xc+st*t_sy*yc;
        b[1] = ct*t_sy;
        b[2] =  st*t_sx;
        b[3] = -ct*t_sy*yc-st*t_sx*xc;
    ...
    for (ienlarge = 0; ienlarge < NENLARGE; ienlarge++)
    {
        ...
        else
        {
            ...
            matchByPos (sxi, seta, ns, gr, gd, gx, gy, ng,
                        r2as, r2as, pMATCHDIST, npmax, mats, matg, &npair);
#ifdef TOPVOTES_TRACE
            printf ("TOPVOTES_TRACE: %d pairs\n", npair);
            printf ("       s   g  %s  arcsec\n",
                    "matched pairs, before least squares fit");
 ...
 *             pMATCHDIST in arcsec
 *  pMATCHDIST  dist. within which image/catalogue star considered same
...
#define CHARS 3000
    ...
    for (ibg = 0; ibg < ng && ! enough; ibg++)
    {
        ...
        for (ibs = 0; ibs < ns && ! enough; ibs++)
        {
            ...
            /* find pairs of distances that are the same, within pMATCHDIST,
            ...
            for (nmc = i = j = 0; i < ns && j < ng;  )
            {
                /* if distances agree then i (image) & j (catalogue) is a new
                   candidate match */
                if (fabs(sdi[i].d-gdi[j].d) < pMATCHDIST)
                {
                    ...
                    for (im = 0; im < nmc; im++)
                    {
                        ...
                        if (fabs(ds-dg) > pMATCHDIST) break;
#ifdef MATCH_TRACE
                        if ( (cptr=strchr(infostr,'\0')) - infostr < CHARS - 99 )
                            sprintf(cptr, "%iok ", im);
                        else sprintf(cptr-9, "........\n");
 ...
 * the distance pMATCHDIST.
 * At present, it isn't checked whether two catalogue stars are within
 * pMATCHDIST of the same image star.  The ip.cfg parameter FSMINSEP actually
 ...
 *             by to make units same as pMATCHDIST
 *  pMATCHDIST  dist. within which image/catalogue star considered same
 *  npmax      maximum number of pairs (= lower of ns, ng)
 *
 *    If a match is found, indices of matching stars are written to
 ...
 */
{
    ...
    for (distsq = pow(pMATCHDIST,2), j = 0; j < ng && nm < npmax; j++)
    {
        for (im = i = 0; i < ns; i++)
        {
             ...
             double *xi, double *eta)
{
            ...
            i++,   rp++,   dp++,   xip++,    etap++)
    {
        ...
        cd = cos(*dp);
        sd = sin(*dp);
        R = 1/(cd*cr*cdc+sd*sdc);
        *xip = R*cd*sr;
        *etap = R*(sd*cdc-cd*cr*sdc);
    }
}
