...
#define LMAXPAIR     25  /* most pairs we use for final fitting */
#define LMINPAIR     4   /* minimum pairs we may use for solution */
...
#define GIDX(i)     ((i)%ng)    /* GSC index from vote array index */
    double smx[LMAXPAIR], smy[LMAXPAIR];  /* list of matched s stars */
    double gmr[LMAXPAIR], gmd[LMAXPAIR];  /* list of matched g stars */
    double resid[LMAXPAIR];  /* residual for each pair */
     ...
     * N.B. this also allows the temp arrays to be sized at LMAXPAIR.
     */
    if (npair > LMAXPAIR)
        npair = LMAXPAIR;
...
#define GNUPLOT_TRACE
#ifdef GNUPLOT_TRACE
        {
            ...
            double xem[LMAXPAIR], yem[LMAXPAIR];
            double mxerr, myerr;
            FILE *fp;
...
{
    FImage *fip = &fim_g;
    double mx[LMAXPAIR], my[LMAXPAIR];
    double ra = v[0];
    double dc = v[1];
    double th = v[2];
    double sx = v[3];
